...
class Engine(object):
    def __init__(self, validate_object, verbose: bool):
    ...
    def __call__(self, fastq_file):
        errors = []
        if self.verbose:
            print(f"Validating matching fastq file {fastq_file}")
...
class FASTQValidatorLogic:
    ...
    def _log(self, message: str) -> Optional[str]:
        if self.verbose:
            print(message)
        return message
    ...
    def validate_fastq_file(self, fastq_file: Path) -> None:
        self._log(f"Validating {fastq_file.name}...")
        self._log(f"    → {fastq_file.absolute().as_posix()}")
    ...
    def validate_fastq_files_in_path(self, paths: List[Path], threads: int) -> None:
        ...
        for path in paths:
            ...
            self._log(f"Added files from {path} to dirs_and_files: {dirs_and_files}")
        file_list = []
        with Manager() as manager:
            ...
            try:
                ...
                pool = Pool(threads)
                engine = Engine(self, verbose=self.verbose)
    ...
    def _find_duplicates(self, dirs_and_files):
        ...
        for data_path, sub_dirs in dirs_and_files.items():
            ...
            for filename, filepaths in files_per_path.items():
                if len(filepaths) > 1:
                    self.errors.append(
                        f"{filename} has been found multiple times during this validation. Locations of duplicates: {filepaths}."
                    )
